...
Coordinate_t CalculateKnotCoordinateChange(Coordinate_t headCoor, Coordinate_t tailCoor);
  ...
  if (inputFile.is_open()) 
  {
    while(getline(inputFile, currentLine))
    {
      ...
      for (int i = 0; i < moves; i++)
      {
        ...
        {
          knots.at(i) = CalculateKnotCoordinateChange(knots.at(i - 1), knots.at(i));
        }
        tailCoors.insert(knots.back());
      }
    }
